<template>
  ...
  >
    <v-card class="deposit-dialog-card tw-rounded-2xl">
        ...
        <v-icon size="20" class="dialog-title-icon tw-shrink-0">mdi-currency-btc</v-icon>
        <span class="tw-truncate">{{ t('wallet.withdrawal.cryptoWithdrawal') }}</span>
            ...
            <v-stepper-window-item :value="1">
              <div class="deposit-step-content">
                <div class="deposit-step-heading-row">
                  <p class="deposit-step-heading tw-mb-0">{{ t('wallet.withdrawal.chooseCryptoAccount') }}</p>
                  <v-btn icon size="small" variant="tonal" color="primary" class="tw-shrink-0 dialog-add-btn" aria-label="Add crypto account"
                    @click="goToAddCryptoAccount">
                    <v-icon size="20">mdi-plus</v-icon>
                  </v-btn>
                </div>
                <div v-if="loadingAccounts" class="tw-py-4 tw-text-center">{{ t('common.loading') }}</div>
                <div v-else-if="cryptoAccounts.length === 0" class="tw-py-4 tw-text-center">{{ t('wallet.withdrawal.noCryptoAccounts') }}</div>
                <div v-else class="withdrawal-account-list tw-flex tw-flex-col tw-gap-3 tw-mb-4">
                  <button v-for="acc in cryptoAccounts" :key="acc.id" type="button" class="withdrawal-account-item"
                    ...
                    <div class="withdrawal-account-item__icon">
                      <component v-if="getCryptoIcon(acc)" :is="getCryptoIcon(acc)" :size="20" />
                      <v-icon v-else size="20">mdi-currency-btc</v-icon>
                    </div>
                    <div class="withdrawal-account-item__body tw-min-w-0 tw-flex-1 tw-text-left">
                      <div class="withdrawal-account-item__title">{{ acc.currency_name || acc.currency_type || acc.crypto_currency_name || t('wallet.withdrawal.crypto') }}</div>
                      <div class="withdrawal-account-item__meta">{{ t('wallet.withdrawal.wallet') }}</div>
                      <div class="withdrawal-account-item__meta tw-font-mono tw-text-xs tw-tracking-wide">{{ formattedCryptoAddress(acc) }}</div>
                    </div>
            ...
            <v-stepper-window-item :value="2">
              <div class="deposit-step-content">
                <div class="deposit-summary-bar">
                  ...
                  <span class="deposit-summary-value">{{ selectedAccount?.currency_name || selectedAccount?.currency_type || selectedAccount?.crypto_currency_name }} •••• {{ (selectedAccount?.wallet_address || selectedAccount?.address || '').slice(-8) }}</span>
                </div>
              ...
              <div class="deposit-step-content">
                <div class="deposit-summary-box">
                  <div class="deposit-summary-row">
                    ...
                    <span><strong>To:</strong> {{ selectedAccount?.currency_name || selectedAccount?.currency_type || selectedAccount?.crypto_currency_name }} •••• {{ (selectedAccount?.wallet_address || selectedAccount?.address || '').slice(-8) }}</span>
                  </div>
                </div>
                <div class="tw-mt-4">
                ...
                <div class="deposit-actions">
                  <v-btn variant="outlined" size="default" rounded="lg" @click="step = 2" :disabled="submitting"
                    class="tw-border-theme-border-strong" color="default">Back</v-btn>
                  <v-btn variant="outlined" size="default" rounded="lg" @click="closeDialog" :disabled="submitting"
                    class="tw-border-theme-border-strong" color="default">Cancel</v-btn>
                  <v-btn color="primary" variant="elevated" size="default" rounded="lg" @click="confirmSubmit"
                    :loading="submitting" :disabled="!withdrawalPassword || withdrawalPassword.length < 4">Confirm</v-btn>
                </div>
              </div>
            </v-stepper-window-item>
          </v-stepper-window>
...
const getCryptoIcon = (acc) => {
  const name = acc?.currency_name || acc?.currency_type || acc?.crypto_currency_name || ''
...
watch(() => props.modelValue, (isOpen) => {
  ...
  } else if (isOpen && cryptoAccounts.value.length === 0 && !loadingAccounts.value) {
    loadingAccounts.value = true
    paymentMethodsApi.getCryptoAccounts().then(res => {
      cryptoAccounts.value = res.data?.data ?? res.data ?? []
    }).catch(e => { console.error(e); showError('Failed to load crypto accounts'); cryptoAccounts.value = [] }).finally(() => { loadingAccounts.value = false })
  }
})

watch(step, (s) => {
  if (s === 1 && cryptoAccounts.value.length === 0 && !loadingAccounts.value) {
    loadingAccounts.value = true
    paymentMethodsApi.getCryptoAccounts().then(res => {
      cryptoAccounts.value = res.data?.data ?? res.data ?? []
    }).catch(e => { console.error(e); showError('Failed to load crypto accounts'); cryptoAccounts.value = [] }).finally(() => { loadingAccounts.value = false })
...
const formattedCryptoAddress = (acc) => {
  const addr = acc.wallet_address || acc.address || ''
  if (!addr) return '—'
...
const goToAddCryptoAccount = () => {
  closeDialog()
  router.push({ path: '/payment-methods', query: { tab: 'crypto' } })
}
...
const confirmSubmit = async () => {
  ...
  try {
    ...
    const payload = { amount: withdrawalAmount.value, password: withdrawalPassword.value, payment_method_id: selectedAccount.value?.id, payment_type: 'crypto' }
