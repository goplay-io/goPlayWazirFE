    ...
    <template v-slot:default>
      <v-card class="deposit-dialog-card tw-rounded-2xl">
        <v-card-title class="deposit-dialog-title tw-border-b">
          ...
          <span class="tw-truncate">{{ t('wallet.deposit.bankDeposit') }}</span>
        ...
        <v-card-text class="deposit-dialog-body">
          ...
          <v-stepper v-model="step" :items="stepperItems" hide-actions :mobile="$vuetify.display.smAndDown"
            ...
            <v-stepper-window>
              ...
              <v-stepper-window-item :value="2">
                <div class="deposit-step-content">
                  ...
                  <p class="deposit-step-heading deposit-step-heading--muted">{{ t('wallet.deposit.chooseBankAccount') }}</p>
                  <div v-if="!loadingParent && parentBankList.length === 0" class="deposit-step-label tw-mb-2">
                    {{ t('wallet.deposit.noBankAccounts') }}
                  </div>
                  <div v-else class="tw-grid tw-grid-cols-2 sm:tw-grid-cols-3 tw-gap-2 tw-mb-2">
                    <button v-for="(acc, idx) in parentBankList" :key="acc.id" type="button"
                  ...
                  <div class="deposit-actions">
                    ...
                    <v-btn color="primary" variant="elevated" size="default" rounded="lg" @click="step = 3"
                      :disabled="!selectedAccount || parentBankList.length === 0">{{ t('wallet.deposit.next') }}</v-btn>
                  </div>
                </div>
              </v-stepper-window-item>
                  ...
                  <div class="deposit-step-three__left">
                    ...
                    <div class="bank-info-fields">
                      <div class="bank-info-field">
                        <p class="bank-info-field__label">{{ t('wallet.deposit.accountHolder') }}</p>
                        <div class="bank-info-field__value" @click="copyBankField(bankInfo.accountHolderName, t('wallet.deposit.accountHolder'))">
                          <span>{{ bankInfo.accountHolderName || '—' }}</span>
                          <v-btn icon="mdi-content-copy" size="x-small" variant="text" class="bank-info-field__copy"
                            @click.stop="copyBankField(bankInfo.accountHolderName, t('wallet.deposit.accountHolder'))"></v-btn>
                        </div>
                      </div>
                      <div class="bank-info-field">
                        <p class="bank-info-field__label">{{ t('wallet.deposit.accountNumber') }}</p>
                        <div class="bank-info-field__value" @click="copyBankField(bankInfo.accountNumber, t('wallet.deposit.accountNumber'))">
                          <span>{{ bankInfo.accountNumber || '—' }}</span>
                          <v-btn icon="mdi-content-copy" size="x-small" variant="text" class="bank-info-field__copy"
                            @click.stop="copyBankField(bankInfo.accountNumber, t('wallet.deposit.accountNumber'))"></v-btn>
                        </div>
                      </div>
                      <div class="bank-info-field">
                        <p class="bank-info-field__label">{{ t('wallet.deposit.ifscCode') }}</p>
                        <div class="bank-info-field__value" @click="copyBankField(bankInfo.ifscCode, t('wallet.deposit.ifscCode'))">
                          <span>{{ bankInfo.ifscCode || '—' }}</span>
                          <v-btn icon="mdi-content-copy" size="x-small" variant="text" class="bank-info-field__copy"
                            @click.stop="copyBankField(bankInfo.ifscCode, t('wallet.deposit.ifscCode'))"></v-btn>
                        </div>
                      </div>
                      <div class="bank-info-field">
                        <p class="bank-info-field__label">{{ t('wallet.deposit.bankName') }}</p>
                        <div class="bank-info-field__value" @click="copyBankField(bankInfo.bankName, t('wallet.deposit.bankName'))">
                          <span>{{ bankInfo.bankName || '—' }}</span>
                          <v-btn icon="mdi-content-copy" size="x-small" variant="text" class="bank-info-field__copy"
                            @click.stop="copyBankField(bankInfo.bankName, t('wallet.deposit.bankName'))"></v-btn>
                        </div>
                      </div>
                  </div>
...
const bankInfo = computed(() => {
  ...
  if (!acc) return { accountHolderName: '', accountNumber: '', ifscCode: '', bankName: '' }
  return {
    accountHolderName: acc.holder_name ?? '',
    accountNumber: acc.account_number ?? '',
    ifscCode: acc.account_ifsc ?? '',
...
async function loadParentBankOptions() {
  loadingParent.value = true
  parentBankList.value = []
  eligibilitySessionId.value = null
  selectedAccount.value = null
  try {
    const res = await getParentPaymentMethods('bank', depositAmount.value)
    const data = res?.data?.data ?? res?.data
    parentBankList.value = Array.isArray(data?.list) ? data.list : []
    eligibilitySessionId.value = getEligibilitySessionId(data)
    return true
  } catch (e) {
    console.error('Failed to load parent bank options:', e)
...
async function goToAccountStep() {
  ...
  if (await loadParentBankOptions()) step.value = 2
}
...
const copyBankField = async (value, label) => {
  ...
  try {
    ...
    showSuccess(t('common.copiedToClipboard', { label }))
  } catch (error) {
    console.error('Failed to copy:', error)
    showError(t('common.copyFailed', { label }))
  }
}
...
<style scoped>
/* Design system: consistent typography and spacing across deposit dialogs */
