      ...
      <div class="tw-rounded-[13px] tw-bg-theme-surface-alt tw-overflow-hidden tw-flex tw-flex-col tw-max-h-[85vh]">
        ...
        <div class="tw-flex tw-items-center tw-justify-between tw-gap-3 tw-p-[18px] tw-border-b tw-border-theme-border">
          <div class="tw-flex tw-items-center tw-gap-2.5">
            <v-icon size="24" class="tw-text-secondary">mdi-heart</v-icon>
            <div class="tw-text-lg tw-font-bold tw-text-theme-text">{{ t('casino.favorites.title') }}</div>
            <v-chip v-if="favoriteGames.length > 0" size="small" color="secondary" variant="tonal">
        ...
        <!-- No Favorites -->
        <div v-else-if="favoriteGames.length === 0" class="tw-text-center tw-py-12 tw-px-6">
          <v-icon icon="mdi-heart-outline" size="64" class="tw-text-theme-text-muted tw-mb-3"></v-icon>
          <p class="tw-text-theme-text-secondary tw-text-lg">{{ t('casino.favorites.noFavorites') }}</p>
          <p class="tw-text-theme-text-muted tw-text-sm tw-mt-2">{{ t('casino.favorites.visitCasino') }}</p>
        </div>
...
// Transform favorite games to have consistent properties
